...
const ChallengeModal = ({ isModalOpen, handleClose, challenge }) => {
  ...
  const postNumber = async () => {
    try {
      ...
      const res = await challengApi.modifyChallenge(
        id,
  ...
  return (
    ...
    >
      <Box className={ChallengesList.ModalBox}>
        <div key={challenge.id}>
          ...
          <Typography sx={{ fontFamily: "Giants-Bold" }}>
            -설명: {challenge.comment}
            <br />
            -주의:{challenge.caution}
            <br />
          ...
          <Typography>
            <p id={ChallengesList.body4}>
              -모집인원:{number}/{challenge.max}명
            </p>
          </Typography>
          <Typography>
            {number >= challenge.max ? (
              ...
              <Button onClick={openAttend} className={ChallengesList.body3}>
                취소하기
              </Button>
            ) : (
              <Button onClick={closeAttend} className={ChallengesList.body3}>
                참여하기
              </Button>
            )}
          </Typography>
