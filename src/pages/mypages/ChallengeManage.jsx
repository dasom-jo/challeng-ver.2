  ...
  return (
    <div className={challengeManage.ChallengeManag}>
      <div className={challengeManage.btn1}>
        <button onClick={closeList} className={challengeManage.btn2}>
          완료
        </button>
        <button onClick={openList} className={challengeManage.btn2}>
          진행중
        </button>
      </div>
      {openChallengeList ? (
        <ChallengeList
          challenges={myChallenge.filter(
            (challengeItem) => !challengeItem.success,
          )}
        />
      ) : (
        <ChallengeList
      ...
      {challenges.map((challengeItem) => (
        ...
        >
          {challengeItem.Challenge && (
            <li
              className={challengeManage.ChallengeLists}
              key={challengeItem.Challenge.id}
            >
