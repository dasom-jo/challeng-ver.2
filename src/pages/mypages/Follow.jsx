  ...
  return (
    <div className={follow.Info2}>
      <div className={follow.followBtn}>
        <Button onClick={openFollow} className={follow.followBtn1}>
          팔로워
        </Button>
        <Button onClick={closeFollow} className={follow.followBtn2}>
          팔로잉
        </Button>
      </div>
    ...
    try {
      ...
      if (res.data.code === 200) {
        setFollowingList((prevList) =>
          prevList.filter((following) => following.id !== id),
        );
      }
    } catch (error) {
      console.error("API 호출 중 에러:", error);
